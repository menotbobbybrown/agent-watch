...
def custom_endpoint() -> tuple[str | None, str | None]:
    endpoint = os.environ.get(CUSTOM_ENDPOINT_VAR)
    if not endpoint:
        # Check config file
        try:
            from config import read_env_file, get_config_file
            env_map = read_env_file(get_config_file())
            endpoint = env_map.get(CUSTOM_ENDPOINT_VAR)
        except ImportError:
            pass
    if not endpoint:
        return None, None
    model = os.environ.get(CUSTOM_MODEL_VAR) or CUSTOM_MODEL_DEFAULT
    return endpoint, model

def transcribe_local(video_path: str, out_dir: Path) -> tuple[list[dict], str]:
    out_dir.mkdir(parents=True, exist_ok=True)
    audio_path = out_dir / "audio.mp3"
    subprocess.run([
        "ffmpeg", "-y", "-i", video_path, "-vn", "-ac", "1", "-ar", "16000", "-b:a", "64k", str(audio_path)
    ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
    
    # Try faster-whisper or openai-whisper in python
    try:
        import whisper
        model_name = os.environ.get("WATCH_LOCAL_WHISPER_MODEL", "small")
        ...
        return segments, f"local openai-whisper ({model_name})"
    except ImportError:
        pass
    ...
    try:
        ...
        return segments, f"local faster-whisper ({model_name})"
    except ImportError:
        pass

    raise SystemExit("Local whisper requires either `pip install openai-whisper` or `pip install faster-whisper`.")
...
GEMINI_ENDPOINT_VAR = "WATCH_GEMINI_API_KEY"

def transcribe_gemini(audio_path: Path, api_key: str) -> tuple[list[dict], str]:
    """Transcribe audio using Google Gemini 1.5 Flash audio API endpoint."""
    ...
    payload = {
        "contents": [{
            "parts": [
                {"text": "Provide a clean, timestamped transcript of this audio clip with format [MM:SS] text."},
    ...
    with urllib.request.urlopen(req) as resp:
        ...
        # Convert response text to segments
        segments = []
        for idx, line in enumerate(text.splitlines()):
            line = line.strip()
            if line:
                segments.append({"start": idx * 5.0, "end": (idx + 1) * 5.0, "text": line})
        return segments, "gemini-1.5-flash"
