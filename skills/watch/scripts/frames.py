...
def merge_frames(primary: list[dict], pinned: list[dict]) -> list[dict]:
    ...
    """
    merged = sorted([*primary, *pinned], key=lambda f: f.get("timestamp_seconds", f.get("timestamp", 0.0)) if isinstance(f, dict) else 0.0)
...
def _dedupe_by_deltas(
    ...
    for cand, thumb in zip(candidates[1:], thumbs[1:]):
        diff = sum(abs(a - b) for a, b in zip(last, thumb)) / len(thumb)
        if diff <= threshold:
            dropped.append(cand)
        else:
            kept.append(cand)
...
def run_ocr_on_frame(frame_path: Path) -> str | None:
    """Extract text from a JPEG frame using tesseract binary, pytesseract, or easyocr if available."""
    if not frame_path.exists():
        return None
    # 1. Try tesseract CLI if installed
    if shutil.which("tesseract"):
        try:
            res = subprocess.run(
                ["tesseract", str(frame_path), "stdout", "--oem", "1", "-l", "eng"],
...
def extract_ocr_for_frames(frames: list[dict]) -> list[dict]:
    """Run OCR across extracted frames and attach `ocr_text` to frames with detected text."""
    has_ocr_tool = bool(shutil.which("tesseract"))
    if not has_ocr_tool:
        try:
