...
**Give ANY AI Agent (Antigravity, OpenCode, Claude Code, Codex, Cursor, Copilot, Gemini) the ability to watch any video.**

Claude Code (recommended — auto-updates via marketplace):
```
/plugin marketplace add menotbobbybrown/agent-watch
/plugin install watch@agent-watch
```

Codex, Cursor, Copilot, Gemini CLI, or any of 50+ [Agent Skills](https://agentskills.io) hosts:
```bash
npx skills add menotbobbybrown/agent-watch -g
```
...
**Diagnose a bug from a video.** Someone sends you a screen recording of something broken. `/watch bug-repro.mov what's going wrong?` The agent watches the recording, finds the frame where the issue appears, describes what's on screen, often catches the cause without you ever opening the file.
...
3. **`ffmpeg` extracts frames at the chosen detail.** `efficient` decodes keyframes only (near-instant); `balanced`/`token-burner` prefer scene-change frames and fall back to the duration-aware uniform sampler when they under-produce. JPEGs are 512px wide by default and clamped to 1998px tall for Agent vision compatibility.
...
7. **Cleanup.** The script prints a working directory at the end. If you're not asking follow-ups, The agent removes it.
...
| **Codex, Cursor, Copilot, Gemini CLI, +50 more** | `npx skills add menotbobbybrown/agent-watch -g` |
| **claude.ai** (web) | [Download `watch.skill`](https://github.com/menotbobbybrown/agent-watch/releases/latest) → Settings → Capabilities → Skills → `+` |
...
Update later with `/plugin update watch@agent-watch`.
...
1. [Download `watch.skill`](https://github.com/menotbobbybrown/agent-watch/releases/latest) from the latest release.
...
```bash
git clone https://github.com/menotbobbybrown/agent-watch.git
...
<a href="https://www.star-history.com/?repos=menotbobbybrown%2Fagent-watch&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=menotbobbybrown/agent-watch&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=menotbobbybrown/agent-watch&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=menotbobbybrown/agent-watch&type=date&legend=top-left" />
 </picture>
</a>

---

[github.com/menotbobbybrown/agent-watch](https://github.com/menotbobbybrown/agent-watch) · [@menotbobbybrown](https://www.youtube.com/@menotbobbybrown) · [Solaris Automation](https://www.solarisautomation.io/) · [LICENSE](LICENSE)
